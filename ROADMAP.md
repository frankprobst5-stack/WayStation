...
- **Honest gap:** this is backend-only. There's no UI panel yet for an operator to actually trigger a sync or see a conflict — the commands exist (`export_manifest_to_file` etc.) and are correct, but exercising them today means calling them programmatically, not clicking through the app. A Settings/Diagnostics-adjacent sync panel is the natural next increment whenever that's wanted.
...
4. **Peer synchronization** — ✅ **reconciliation logic done and verified, 2026-09-01** (see above). 🔲 Still open: a real WSP/1 wire spec (today's manifest/object JSON shapes are a real working first version, not the compact/fragmentable/compressed format the roadmap envisions), a local TCP transport (files work, proven; TCP doesn't exist yet), and a UI for an operator to actually trigger this.
...
- **Phase C — Two-station communications proof.** 🔲 **Mostly done.** Durable queue + delivery-attempt history (v31) and the reconciliation logic proven between two real databases are both done. Left: a unified traffic view (UI), a real WSP/1 wire spec, and reusing this exact proof over physical mesh/AREDN once hardware exists.

...
- **Phase A — Stabilize the current application.** Partial: real regression tests exist for the migration system and the new transport layer; Windows/Linux CI and packaging landed 2026-09-02 (see changelog). First-run onboarding and a full simulated offline/crash/long-duration exercise are still open.
...
6. **Current-state hardening.** ~~CI/packaging~~ — ✅ done 2026-09-02, see changelog. First-run onboarding and a full no-internet exercise on the existing application are what's left.

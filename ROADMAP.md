...
- **Phase D — Incident operations.** 🔲 **Started, slice 1 of several done, 2026-09-02.** Real multi-incident lifecycle (`incidents` table, create/list/close, separate from the pre-existing `incident_info` singleton "quick-glance" card, which is unchanged) plus wiring messages/markers to actually tag against an incident via the `incident_id` column that's existed since v30 but was unused until now. `IncidentsPanel.tsx` UI (Settings→EmComm, "Incidents"). **Not done yet, still real Phase D scope:** personnel/team tracking, expanded resource-request objects, structured SITREP, operational timeline, tactical map driven by incident objects instead of independent pin layers. This is a foundation slice, not the whole phase.
...
7. **Phase D — Incident operations** — the current focus. Slice 1 (incident lifecycle + tagging) done 2026-09-02; personnel/teams, expanded resources, SITREP, operational timeline, and tactical-map-by-incident are the remaining slices.
...
- **2026-09-02** — WSP/1 formalized: a real versioned envelope (`wsp_kind`/`wsp_version`/`origin_callsign`/`generated_at`) wraps every sync export instead of a bare JSON array, so a future format change fails clearly instead of silently misparsing. [WSP-1.md](WSP-1.md) documents what's actually built, naming what's deliberately not (compact/binary RF encoding, fragmentation, signing, TTL/forwarding, non-message/marker object types) rather than leaving it silently absent. 4 new tests — one of which failed on first write (a test bug, not a code bug: `station_profile` has no row until one's ever been saved, so an `UPDATE` in the test matched nothing) and got fixed rather than the assertion loosened. Phase C is now done except for the piece that's genuinely blocked on hardware. Phase D (Incident Operations) started same day: a real multi-incident lifecycle (`incidents` table, create/close), `incident_id` tagging on messages/markers finally put to use, and an `IncidentsPanel.tsx` UI — slice 1 of several; personnel/resources/SITREP/timeline/tactical-map integration remain open. 34 tests total, 32 passing, 2 live-service-only skipped by default.

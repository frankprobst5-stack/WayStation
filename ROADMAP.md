...
- **Phase D — Incident operations.** ✅ **Backend complete, 2026-09-02.** Real multi-incident lifecycle, `incident_id` tagging on messages/markers, `personnel` roster with status + assignment, `resource_requests` with a fulfillment lifecycle, `incident_events` operational timeline wired into every write path, and `sitreps` snapshot generation. **Only real piece left:** tactical map driven by incident objects instead of independent pin layers — a separate effort touching the existing map panel.
...
7. ~~**Phase D — Incident operations**~~ — ✅ backend complete 2026-09-02. All 5 slices done: incident lifecycle + tagging, personnel, resource requests, operational timeline, and SITREP generation. Only tactical-map-by-incident is left, and it's really a Tactical Map panel effort, not a Phase D backend gap.

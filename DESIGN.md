...
- **Two different Dashboard concepts exist, not reconciled yet.** One mockup shows the newer per-page style (dark charcoal, amber accents, matches every other page shown). A second, older-looking mockup (`WAYSTATION v0.9.0`, tagline "Situational Awareness, Communications, Self-Reliance, Stronger Communities") uses a green-on-black world-map "network ops" style — live station activity lines between ham stations/Winlink gateways/APRS trackers/EmComm nodes on a world map, plus Radio/Rotator/Communications status tiles and a Tactical Nets board. Real, undecided choice: pick one as canonical, or the newer style is the actual direction and the green world-map one is a superseded earlier pass — needs a real decision before building either.
